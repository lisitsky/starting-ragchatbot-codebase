...
class TestEndpointErrorInjection:
    ...
    def test_endpoint_all_exceptions_masked(self):
        ...
        with patch.object(RAGSystem, "query", side_effect=ValueError("Specific error message")):
            ...
            assert (
                "ValueError" in detail
            ), f"Error detail should include exception type, got: {detail}"
            assert (
                "Specific error message" in detail
            ), f"Error detail should include message, got: {detail}"
    ...
    def test_endpoint_chromadb_error(self):
        ...
        with patch.object(RAGSystem, "query", side_effect=Exception("ChromaDB connection error")):
            response = client.post("/api/query", json={"query": "What is MCP?", "session_id": None})
            ...
            assert (
                "ChromaDB connection error" in detail
            ), f"Error detail should include message, got: {detail}"
