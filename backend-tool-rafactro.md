...
Current behaviour:
- Claude makes 1 tool call -> tools are remoed from API params -> final response
...
2. Claude: get course aoutline for course X -> gets title of lesson 4
...
- Write tests that verify the external behaviour (API call made, tools executed, results returned) rather that internal state details.


Use two parallel subagents to brainstoorm possible plans. Do not implement any code.